...
/// * [DropdownMenuEntry], which is used to build the [MenuItemButton] in the [CustomDropdownMenu] list.
    ...
    return entries
        .where((DropdownMenuEntry<T> entry) =>
            (entry.label.toLowerCase().contains(filterText) ||
                entry.value.toString().toLowerCase().contains(filterText)))
        .toList();
  }
    ...
    Widget menuAnchor = MenuAnchor(
      ...
      builder:
        ...
        if (widget.expandedInsets != null) {
          ...
          // on its parent width. So we don't need to use `_CustomDropdownMenuBody` to
